...
function printResult(result: any) {
  ...
  result.forEach((item) => {
    console.log(chalk.hex('#00ffff').bold(`Name: ${item.funcName}`));
    console.log(chalk.hex('#ffd700').bold(`Desc: ${item.description}`));
    console.log(chalk.hex('#ffd700').bold(`Price: ${item.price}`));
    console.log(chalk.hex('#ffd700').bold(`Params: ${item.params}`));
    console.log(chalk.hex('#ffd700').bold(`Usage: ${item.usage}\n`));
  });
}
          ...
          keyManager.loadCredentials()
            .then((key) => {
              ...
              client.runFunction(funcName, JSON.stringify(paramsJson)) // TODO: wallet vuoto
                ...
                .catch(() => {
                  console.log(chalk.red.bold('Cannot run function. It may not exist or may not be available.'));
                });
            })
            .catch(() => {
